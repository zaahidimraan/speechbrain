...
class Enhancement(sb.Brain):
    ...
    def fit_batch(self, batch):
        ...
        # Extract the audio signal from the batch
        signal = batch.noisy_sig[0][0, :]  # Get first (and only) sample from batch

        # Normalize for better audio quality (optional)
        signal = signal / torch.max(torch.abs(signal)) 

        import random
        import string

        def generate_random_filename(length=4, ext=".wav"):  # Default 10-character length, .wav extension
            """Generates a random filename with the specified length and extension."""
            letters = string.ascii_lowercase + string.digits  # Use lowercase letters and digits
            return ''.join(random.choice(letters) for i in range(length)) + ext

        # Example usage
        name = generate_random_filename(5)
        # Save to WAV file
        filename = 'noisy_'+name

        save_path = os.path.join('dataO', filename)
        torchaudio.save(save_path, signal.unsqueeze(0).cpu(), hparams["sample_rate"])
        # Extract the audio signal from the batch
        signal = batch.clean_sig[0][0, :]  # Get first (and only) sample from batch
